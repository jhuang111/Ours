...
class PotNet(nn.Module):
    ...
    def forward(self, data, print_data=False):
        ...
        # 边索引
        # print('running in PotNet........')
        # print('data information')
        # print(data)
        # print(data.batch)
        # print(data.edge_index)
        # print(data.edge_attr)
        # print(data.inf_edge_index)
        # print(data.inf_edge_attr)
        # print('new information')
        # print(data.tuple_edge_index)
        # print(data.edge_4)
        # print(data.dist)
        # print(data.theta)
        # print(data.phi)
        ...
        # TODO: Ⅲ. ②对源节点和周期节点的node_features做一致性处理。验证节点顺序没有发现变化 即0 1 相同
        # print('node_features after embedding')
        # print(node_features)
        # node_features_clone = node_features.clone()
        # node_features = assi01(node_features_clone)
        # print('node_features after assi')
        # print(node_features)
        # TODO：Ⅱ 添加对于Data的处理（copy ComENet），得到TBF和SBF
        if cal_4edge:
            ...
            # print(TBF)
            # 检查是否有 NaN 值
            # has_nan = torch.isnan(TBF).any()
            # # 检查是否有无穷大的值
            # has_inf = torch.isinf(TBF).any()
            # if has_nan or has_inf:
            #     print("error in TBF。")
            # else:
            #     print("error not in TBF")
            # TBF_repeat = TBF.repeat(1, 1 + int(round(256 / 12)))[:, :256]
            # print(TBF.size())  # torch.Size([13416, 12])
            SBF = self.feature2(dist, tau)
            # print('SBF')
            # print(SBF)
            # SBF_repeat = SBF.repeat(1, 1 + int(round(256 / 6)))[:, :256]
            # T_SBF = torch.cat((TBF, SBF), dim=1)
            # TSBF_repeat = T_SBF.repeat(1, 1 + int(round(256 / 18)))[:, :256]
            # print(SBF.size())  # torch.Size([13416, 6])
            # print(data.edge_4_u.size())
            # print(data.edge_4_v.size())
            # print(data.edge_4.size())
            feature1 = self.lin_feature1(TBF)
            # 检查是否有 NaN 值
            # has_nan = torch.isnan(feature1).any()
            # # 检查是否有无穷大的值
            # has_inf = torch.isinf(feature1).any()
            # if has_nan or has_inf:
            #     print("error in feature1。")
            # else:
            #     print("error not in feature1")
            # print('TBF after linear:')
            # print(feature1)
            feature2 = self.lin_feature2(SBF)
            # print('SBF after linear:')
            # print(feature2)
            # feature1 = self.infinite_bn(F.softplus(feature1))
            # print('TBF after bn:')
            # print(feature1)
            # feature2 = self.infinite_bn(F.softplus(feature2))
            # print('SBF after bn:')
            # print(feature2)
            tuple_index = data.tuple_edge_index
            # print('tuple_index:')
            # print(tuple_index)
            # print(tuple_index.size())
            # print(tuple_index[0][48])
            # print(tuple_index[1][48])
            # print(tuple_index[0][49])
            # print(tuple_index[1][49])
            # period_edge_index = torch.cat([tuple_index, tuple_index], 1)  # 拼接边索引
            # period_edge_features = torch.cat([feature1, feature2], 0)  # 拼接边特征
            # print(period_index.size())  # torch.Size([2, 13416])
        # if not self.config.transformer:  # 默认执行，config.transformer默认false
        # edge_index = torch.cat([data.edge_index, inf_edge_index], 1)  # 拼接边索引
        # dge_features = torch.cat([edge_features, inf_edge_features], 0)  # 拼接边特征
        # TODO: Ⅲ. ①对下面函数进行修改，添加参数TBF和SBF，网络中需要添加相应的卷积层
        # new_edge_index = torch.cat([period_edge_index, period_edge_index, edge_index], 1)  # 拼接边索引
        # new_edge_features = torch.cat([feature1, feature2, edge_attr], 0)  # 拼接边特征
        # print(period_index.dtype)
        # print(inf_edge_index.dtype)
        ...
        # print('edge index after cat:')
        # print(edge_index)
        # print('edge_features after cat:')
        # print(edge_features)
        # ComENet的处理方式：
        if use_ComENet:
        ...
        for i in range(self.config.conv_layers):
            if self.config.transformer:  # 默认False
                ...
                # TODO: Ⅲ. ①对下面函数进行修改，添加参数TBF和SBF，网络中需要添加相应的卷积层
                node_features = self.conv_layers[i](node_features, tuple_edge_index, tuple_edge_features)
                # print(node_features)
                # print('node_features after conv:')
                # print(node_features)
                # node_features_clone = node_features.clone()
                # node_features = assi01(node_features_clone)
                # print('node features after assi:')
                # print(node_features)
                # TODO: Ⅲ. ②交互模块每次结束对源节点和周期节点的node_features做一致性处理
        # 全局池化
        features = global_mean_pool(node_features, data.batch)
        # TODO:池化结束后是否要做一致性处理？
        # print('features after pooling:')
        # print(features)
        # 接一个全连接层
