...
class torsion_emb(torch.nn.Module):
    ...
    def forward(self, dist, theta, phi):
        ...
        rbf = torch.stack([f(dist) for f in self.bessel_funcs], dim=1)
        # 检查是否有 NaN 值
        # has_nan = torch.isnan(rbf)
        # if torch.any(has_nan):
        #     print("包含 NaN 的位置索引：", torch.nonzero(has_nan).flatten().tolist())
        # 检查是否有无穷大的值
        # has_inf = torch.isinf(rbf).any()
        # if has_nan.any() or has_inf:
        #     print("error in rbf1")
        # else:
        #     print("error not in rbf1")
        sbf = torch.stack([f(theta, phi) for f in self.sph_funcs], dim=1)
        # print(sbf)
        # print(sbf.size())
        # 检查是否有 NaN 值
        # has_nan = torch.isnan(sbf).any()
        # # 检查是否有无穷大的值
        # has_inf = torch.isinf(sbf).any()
        # if has_nan or has_inf:
        #     print("error in sbf1")
        # else:
        #     print("error not in sbf1")

        n, k = self.num_spherical, self.num_radial
        rbf = rbf.view((-1, n, k)).repeat_interleave(self.degreeInOrder, dim=1).view((-1, n ** 2 * k))
        # 检查是否有 NaN 值
        # has_nan = torch.isnan(rbf).any()
        # # 检查是否有无穷大的值
        # has_inf = torch.isinf(rbf).any()
        # if has_nan or has_inf:
        #     print("error in rbf2")
        # else:
        #     print("error not in rbf2")
